-- AlterTable
ALTER TABLE `ourstorycard` ADD COLUMN `headingLine1` TEXT NULL,
    ADD COLUMN `headingLine2` TEXT NULL,
    ADD COLUMN `sinceLabel` VARCHAR(191) NOT NULL DEFAULT 'Since 2014',
    ADD COLUMN `storyHtml` TEXT NULL;

UPDATE `ourstorycard`
SET `headingLine1` = 'Built by the sea,',
...
WHERE `headingLine1` IS NULL
    OR `headingLine2` IS NULL
    OR `storyHtml` IS NULL;

ALTER TABLE `ourstorycard` MODIFY `headingLine1` TEXT NOT NULL,
    MODIFY `headingLine2` TEXT NOT NULL,
    MODIFY `storyHtml` TEXT NOT NULL;
